<flux:modal wire:model="showCreateModal" class="space-y-4 border-0 bg-background! mx-auto w-full max-w-[96vw] sm:max-w-md md:max-w-lg lg:max-w-2xl p-3 sm:p-4 rounded-none sm:rounded-xl overflow-y-auto max-h-[90vh]">
    ...
    <form wire:submit.prevent="createExpense" class="space-y-4">
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-4">
            <flux:button variant="outline" wire:click="$set('showCreateModal', false)" class="w-full sm:w-auto">{{ __('Cancelar') }}</flux:button>
