<flux:modal wire:model="showPaymentHistoryModal" class="space-y-6 mx-auto border-0 bg-background! w-full max-w-[96vw] sm:max-w-md md:max-w-lg lg:max-w-3xl p-3 sm:p-4 rounded-none sm:rounded-xl overflow-y-auto max-h-[90vh]">
            ...
            <div class="bg-red-50 border border-red-200 rounded-md p-3 sm:p-4">
                <div class="flex">
                    <div class="flex-shrink-0">
                        <svg class="h-5 w-5 sm:h-5 sm:w-5 text-red-400" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="ml-auto pl-3">
                        ...
                        >
                            <svg class="h-5 w-5 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg overflow-x-auto hidden md:block">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">
                                        Fecha
                                    </th>
                                    <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider">
                                        Monto
                                    ...
                                    <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">
                                        Usuario
                                    </th>
                                    <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">
                                        Ruta
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($selectedSale->payments->sortByDesc('payment_date') as $payment)
                                    <tr>
                                        <td class="px-3 sm:px-4 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                                            {{ \Carbon\Carbon::parse($payment->payment_date)->format('d/m/Y') }}
                                        </td>
                                        <td class="px-3 sm:px-4 py-2 whitespace-nowrap text-xs sm:text-sm font-medium text-gray-900">
                                            ${{ number_format($payment->amount, 2) }}
                                        </td>
                                        <td class="px-3 sm:px-4 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                                            {{ $payment->payment_method_label }}
                                        </td>
                                        <td class="px-3 sm:px-4 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 hidden md:table-cell">
                                            {{ $payment->user->name ?? 'N/A' }}
                                        </td>
                                        <td class="px-3 sm:px-4 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 hidden lg:table-cell">
                                            {{ $payment->route->title ?? 'N/A' }}
                                        </td>
                                    </tr>
                                    @if($payment->notes)
                                        <tr class="bg-gray-50">
                                            <td colspan="5" class="px-3 sm:px-4 py-2 text-xs sm:text-sm text-gray-600">
                                                <strong>Notas:</strong> {{ $payment->notes }}
                                            </td>
                                        </tr>
                                    @endif
