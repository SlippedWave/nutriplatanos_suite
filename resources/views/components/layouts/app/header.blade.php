...
<body class="min-h-screen">
    ...
    <flux:header container class="border-b border-slate-50 shadow-lg ">
        ...
        <flux:navbar class="hidden lg:flex ml-8">
            @if ($role == 'admin' || $role == 'coordinator')
                ...
                <flux:navbar.item icon="currency-dollar" :href="route('accounting.index')"
                    :current="request()->routeIs('accounting.*')" wire:navigate>
                    {{ __('Contabilidad') }}
                </flux:navbar.item>
                <flux:navbar.item icon="chart-bar" :href="route('resources.index')"
        ...
        <flux:dropdown position="top" align="end" class="hidden md:block">
            <flux:profile class="cursor-pointer hover:bg-slate-100! " :initials="auth()->user()->initials()"
                icon-trailing="chevron-down" :name="auth()->user()->name" avatar:name="auth()->user()->name"
            ...
            <flux:menu class="w-55 bg-background! border! border-gray-200! shadow-lg! rounded-lg! space-y-2! p-2!">
                ...
                <div class="">
                    <div class="flex items-center gap-3">
                        <span
                            class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-banana-100 text-banana-900 font-medium text-sm">
                            {{ auth()->user()->initials() }}
                        </span>
    ...
    <!-- Mobile Sidebar -->
    <flux:sidebar stashable sticky class="lg:hidden border-e border-gray-200! bg-background! shadow-lg! gap-2!">
        ...
        <div class="p-1!">
            <flux:navlist variant="outline">
                ...
                @if (auth()->user()->role == 'admin' || auth()->user()->role == 'coordinator')
                    ...
                    <flux:navlist.item icon="currency-dollar" :href="route('accounting.index')"
                        :current="request()->routeIs('accounting.*')" wire:navigate>
                        {{ __('Contabilidad') }}
                    </flux:navlist.item>
        ...
        <div class="p-2 border-t border-gray-100">
            <div class="flex items-center gap-3 mb-4">
                <span
                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[var(--color-primary-100)] text-[var(--color-primary-700)] font-medium">
                    {{ auth()->user()->initials() }}
                </span>
                <div class="flex-1 min-w-0">
            ...
            <flux:navlist variant="outline" class="space-y-1">
                <flux:navlist.item :href="route('settings.profile')" icon="cog" wire:navigate class="w-full">
                    {{ __('Configuración') }}
                </flux:navlist.item>
            </flux:navlist>
